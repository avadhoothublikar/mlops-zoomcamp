...
def prep_db():
    with psycopg.connect("host=localhost port=5432 user=postgres password=example", autocommit=True) as conn:
        ...
        with psycopg.connect("host=localhost port=5432 user=postgres password=example dbname=test") as conn:
...
def calculate_metrics_postgresql(curr, i):
    ...
    predict_drift = result['metrics'][0]['result']['drift_score']
    num_drifted_columns = result['metrics'][1]['result']['number_of_drifted_columns']
    share_missing_values = result['metrics'][2]['result']['current']['share_of_missing_values']

    curr.execute("INSERT INTO dummy_metrics (timestamp, prediction_drift, num_drifted_columns, share_missing_values) VALUES (%s, %s, %s, %s);", (begin + datetime.timedelta(i), predict_drift, num_drifted_columns, share_missing_values))

@flow
def batch_monitoring_backfill():
    prep_db()
    last_send = datetime.datetime.now(pytz.utc) - datetime.timedelta(seconds=10)
    with psycopg.connect("host=localhost port=5432 user=postgres password=example dbname=test", autocommit=True) as conn:
        for i in range(0, 27):
            ...
            if seconds_elapsed < SEND_TIMEOUT:
                time_to_sleep = SEND_TIMEOUT - seconds_elapsed
